// Galerie 3D – Voyage historique (images devant cadre + logs + limites caméra)
...
controls.minDistance = 3.0;     // zoom minimum
controls.maxDistance = 24.0;    // zoom maximum
controls.minPolarAngle = 0.12;  // évite de passer sous l'horizon
...
function loadTextureAbs(url, onOk, onErr){
  ...
  texLoader.load(
    abs,
    (t)=>{ t.colorSpace=THREE.SRGBColorSpace; console.log('[image] ok:', abs); onOk(t); },
    undefined,
    (e)=>{ console.warn('[image] fail:', abs, e); onErr?.(e); }
  );
}

// ----- création d’un “tableau” cliquable sur le mur -----
const POSTER_MAX_W = 2.6, POSTER_MAX_H = 1.7;
const FRAME_THICK  = 0.008; // cadre ULTRA fin
...
async function addPosterAndLabel({titre, periode, vignette, url, couleur}, wallPos, rotY){
  ...
  // Sécurité ordre de rendu & anti z-fighting
  frame.renderOrder  = 0;
  poster.renderOrder = 10;
  poster.material.polygonOffset = true;
  poster.material.polygonOffsetFactor = -1;
  poster.material.polygonOffsetUnits  = 1;
...
// ----- objets d’époque (socle + primitive simple) -----
